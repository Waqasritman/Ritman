package ritman.wallet.MoneyTransferModuleV.viewmodels;
...
public class BankTransferViewModel extends ViewModel {
    ...
    public LiveData<NetworkResource<RitmanPaySendResponse>> getSummary(RitmanPaySendRequest request, String userName) {
        MutableLiveData<NetworkResource<RitmanPaySendResponse>> data = new MutableLiveData<>(); // receiving
        Call<RitmanPaySendResponse> call = restApi.getSummary(RestClient.makeGSONRequestBody(request)
                , userName); // rest api declaration
        //init
        appExecutors.networkIO().execute(() -> call.enqueue(new retrofit2.Callback<RitmanPaySendResponse>() {
            @Override
            public void onResponse(Call<RitmanPaySendResponse> call1, Response<RitmanPaySendResponse> response) {
                if (!response.isSuccessful() && response.errorBody() != null) {
                    JSONObject jsonObject = null;
                    try {
                        jsonObject = new JSONObject(response.errorBody().string());
                        ...
                        RitmanPaySendResponse model = new RitmanPaySendResponse();
                        model.responseCode = 500;
                        model.description = message;
                        data.postValue(NetworkResource.unSuccess(R.string.error_fatal, model));
                    } catch (JSONException | IOException e) {
            ...
            @Override
            public void onFailure(Call<RitmanPaySendResponse> call1, Throwable t) {
                RitmanPaySendResponse temp = new RitmanPaySendResponse();
                if (t instanceof ConnectException) {
                    // COMPLETED handle no internet access case
