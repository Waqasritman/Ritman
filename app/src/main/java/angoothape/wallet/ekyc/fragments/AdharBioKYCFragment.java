...
import angoothape.wallet.databinding.FragmentAdharBioKYCBinding;
...
public class
AdharBioKYCFragment extends BaseFragment<FragmentAdharBioKYCBinding> implements MFS100Event
        , OnDecisionMade {
    ...
    protected void setUp(Bundle savedInstanceState) {
        ...
        ArrayAdapter aa = new ArrayAdapter(getActivity(), android.R.layout.simple_list_item_1, arrayFingerTypeThumb);
        aa.setDropDownViewResource(android.R.layout.simple_list_item_1);

        ArrayAdapter adptDevice = new ArrayAdapter(getActivity(), android.R.layout.simple_list_item_1, arraydeviceType);
        aa.setDropDownViewResource(android.R.layout.simple_list_item_1);
    ...
    @OnClick({R.id.btnDeviceInfo, R.id.btnCapture, R.id.btnAuthRequest, R.id.btnReset, R.id.btnSubmit, R.id.spinnerFingerType})
    ...
    private void getFingerType() {
        spinnerFingerType.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                //  FingerType = parent.getItemAtPosition(position);
                spinnerFingerType.getSelectedItem().toString();
            }
    ...
    public void onCheckboxClicked(View view) {
        ...
        if (checked) {
            ...
            if ((pos + 1) > fingerCount) {
                ...
                Toast.makeText(getActivity(), "Please Select Total Finger Count Proper", Toast.LENGTH_LONG).show();
            }
        } else {
            fingerCount--;
...
//        String url = "http://developer.uidai.gov.in/uidauthserver/";
    ...
    void getBioData() {
        ...
        request.kyc_token = viewModel.kycToken.getValue();
        ...
        request.serial_number = "3782822";
        request.session_key = "";//optional
        request.time_stamp = "";//optional
        request.type = deviceTypeItem;
        request.version_number = "";//// optional // mandatory for morpho device
        ...
        request.wadh_value = viewModel.wadh.getValue();
        ...
        viewModel.YBAdharBioKYC(aeRequest, KeyHelper.getKey(getSessionManager().getMerchantName()).trim(), KeyHelper.getSKey(KeyHelper
...
//                        Log.e("FingerISOTemplate", Base64.encodeToString(fingerData.ISOTemplate(), Base64.DEFAULT));
    ...
    public int getLayoutId() {
        return R.layout.fragment_adhar_bio_k_y_c;
    }
